...
// Limits
const MAX_ITEMS_PER_RUN = process.env.MAX_ITEMS_PER_RUN
	? parseInt(process.env.MAX_ITEMS_PER_RUN)
	: 15;
...
`;
	...
	try {
		...
		const bestMatch = Object.keys(EMOJI_MAP).find(
			(c) =>
...
async function sendToTelegram(title, summary, category, link) {
	...
	const body = {
		...
		reply_markup: {
			inline_keyboard: [[{ text: "🔗 Ler matéria completa", url: link }]],
		},
	};
...
async function processBatch() {
	...
	for (let i = 0; i < shuffledFeeds.length; i += CONCURRENCY_LIMIT) {
		...
		const results = await Promise.all(
			batch.map((source) => processFeed(source, historySet)),
		);
