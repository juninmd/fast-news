...
const DB_PATH =
	...
	path.join(__dirname, "../../../data/news_embeddings.db");
...
export function upsertArticleToSqlite(
...
export function searchSimilarInSqlite(
  ...
  `)
	...
	return scored
		...
		.slice(0, limit)
		.map(
			({ embedding: _e, published_at: _p, image_url: _i, ...rest }) =>
...
export function getSqliteStats(): {
	...
	const store = getSqliteDb();
	const total = (
		store.prepare("SELECT COUNT(*) as c FROM articles").get() as { c: number }
	).c;
