...
function getQueue(): Bull.Queue<TelegramQueueJob> {
  if (!queue) {
    try {
      queue = new Bull<TelegramQueueJob>(config.telegramQueue.name, config.redisUrl, {
        limiter: {
      ...
      throw new Error(`[TelegramQueue] Failed to create queue "${config.telegramQueue.name}": ${(err as Error).message}`);
    }
  }
  return queue;
}
...
export async function startTelegramQueueWorker(): Promise<void> {
  ...
  workerStartPromise = (async () => {
    ...
    q.on('failed', (job, err) => {
      console.error(`[TelegramQueue] Job ${job?.id ?? 'unknown'} failed:`, err.message);
    });
    q.on('completed', (job) => {
...
export async function enqueueTelegramPosts(articles: TelegramArticle[]): Promise<number> {
  if (!config.telegramEnabled || !config.telegramBotToken || !config.telegramChatIds.length) {
    console.warn('[TelegramQueue] Skipping enqueue: Telegram is disabled or missing credentials/chat IDs.');
    return 0;
  }
  if (!articles.length) {
  ...
  if (failed.length > 0) {
    console.error(`[TelegramQueue] Failed to enqueue ${failed.length}/${articles.length} article(s).`);
  }
...
export async function retryFailedTelegramPosts(limit = 100): Promise<number> {
  ...
  for (const job of failed) {
    await job.retry().catch((err) => {
      console.error(`[TelegramQueue] Retry failed for ${job.id}:`, (err as Error).message);
    });
  }
