...
Conteúdo: ${(article.content ?? '').slice(0, 1500)}`;
    ...
    const { object } = await generateObject({ model, schema: StorySchema, prompt: contextPrompt });
    ...
    await query(
      `UPDATE news_stories SET
       ...
       WHERE id = $7`,
      [object.title, object.summary, object.impactLevel, object.worldImpact, object.financialSignal, object.affectedAssets, storyId]
    );
    ...
    await query(
      `INSERT INTO story_timeline (story_id, article_id, event_type, headline, what_changed, occurred_at)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [storyId, newArticleId, object.eventType, article.title, object.whatChanged, artPublished.rows[0]?.published_at ?? new Date().toISOString()]
    );
  } catch (err) {
