...
import { SEPARATOR, FLAG_LABEL, BIAS_LABEL, SCORE_EMOJI, escapeHtml } from './telegram_format.js';

export async function generateSummary(title: string, content: string, model?: Awaited<ReturnType<typeof getFastModel>>): Promise<string> {
  try {
...
export async function generateContext(
  ...
  try {
    ...
    const isSerious = category ? seriousCategories.includes(category.toLowerCase()) : false;
...
Título: ${title}\nConteúdo: ${content.slice(0, 4000)}`;
    ...
    const looksPortuguese = /\b(do|da|de|em|com|que|foi|para|uma|um|no|na)\b/i.test(result);
...
export function buildCredibilityBlock(article: {
  ...
  const lines: string[] = [`${emoji} <b>Credibilidade: ${score}/10</b>${article.isMilitant ? '  ·  📢 Panfletário' : ''}`];
...
export async function fetchRelatedArticles(articleId: string, category: string, limit = 2): Promise<Array<{ title: string; url: string; source: string }>> {
  try {
