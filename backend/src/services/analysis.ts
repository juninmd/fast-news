...
export async function analyzeTopicWithRAG(
	topic: TrackedTopic,
): Promise<string> {
	...
	const prompt = ANALYSIS_PROMPT.replace("{topic}", topic.name)
		.replace("{ragContext}", ragContext)
		.replace(
			"{recentNews}",
			recentArticles || "Sem notícias recentes disponíveis.",
		);
	...
	if (analysisId)
		await cacheSet(
			`analysis:latest:${topic.id}`,
			{ id: analysisId, content: text },
			3600 * 6,
		);
...
export async function getAllTrackedTopics(): Promise<TrackedTopic[]> {
	const result = await query<TrackedTopic>(
		`SELECT DISTINCT ON (name) id, name, description, keywords
		 FROM tracked_topics WHERE is_active = TRUE
		 ORDER BY name, id`,
	);
	return result.rows;
}

export async function getTopicLatestAnalysis(
	topicId: string,
): Promise<string | null> {
