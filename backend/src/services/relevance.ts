...
- 1-3: trivial, rotineiro, entretenimento sem impacto, horóscopo, curiosidade
...
export async function scoreRelevance(
	...
	try {
		...
		const res = await generateObject({ model, schema: RelevanceSchema, prompt });
		object = res.object;
	} catch {
		const fallback = await getCloudFallbackModel();
		if (!fallback) {
			console.warn(`[relevance] No model available for ${articleId}, defaulting to 5`);
			return 5;
		}
		try {
			const res = await generateObject({ model: fallback, schema: RelevanceSchema, prompt });
			object = res.object;
		} catch (err) {
			console.warn(`[relevance] Fallback failed for ${articleId}: ${(err as Error).message}`);
			return 5;
		}
	}

	await query(
		`UPDATE news_articles SET relevance_score = $1 WHERE id = $2`,
		[object.relevanceScore, articleId],
	).catch(console.error);
