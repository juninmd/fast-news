...
ragRouter.get('/search', async (req: Request, res: Response) => {
  ...
  try {
    ...
    try {
      ...
      return res.json({ query: q, total: results.length, results, graph: { nodes: [], edges: [] } });
    } catch {
...
async function buildSearchGraph(articleIds: string[]): Promise<{
  nodes: Array<{ id: string; title: string; source: string; category: string; publishedAt: string | null; url: string; sentiment: number }>;
  edges: Array<{ source: string; target: string; similarity: number; relationType: string }>;
}> {
  if (articleIds.length === 0) return { nodes: [], edges: [] };
  const nodes = await query<{
    id: string; title: string; source: string; category: string;
    publishedAt: string | null; url: string; sentiment: number;
  }>(
    `SELECT id, title, source, category, published_at AS "publishedAt", url, sentiment
     FROM news_articles WHERE id = ANY($1::uuid[])`,
    [articleIds]
  );
  const edges = articleIds.length >= 2
