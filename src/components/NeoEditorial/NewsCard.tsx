import { Clock, Bookmark, Share2, Sparkles, CheckCircle2 } from 'lucide-react';
...
export function NewsCard({
  ...
  const catClass = categoryColors[category as keyof typeof categoryColors] ?? categoryColors.default;
  ...
  return (
    ...
    >
      {/* Image */}
      {imageUrl && (
        <div className={`relative overflow-hidden ${variant === 'compact' ? 'h-32' : variant === 'featured' ? 'h-64' : 'h-44'}`}>
          <img
            src={imageUrl}
            alt=""
            loading="lazy"
          ...
          <div className="absolute top-3 left-3">
            <span className={`px-2 py-0.5 rounded-full text-xs font-mono uppercase tracking-wider border ${catClass} backdrop-blur-sm`}>
              {category}
            </span>
          </div>
          {read && (
      ...
      <div className={`p-5 ${variant === 'featured' ? 'p-6' : ''}`}>
        ...
        {!imageUrl && (
          <div className="flex flex-wrap items-center gap-1.5 mb-3">
            <span className={`px-2 py-0.5 rounded-full text-xs font-mono uppercase tracking-wider border ${catClass}`}>
              {category}
            </span>
            {company && (
              <span className="text-xs text-text-secondary">{company}</span>
            )}
            {read && <CheckCircle2 className="w-3 h-3 text-text-secondary/50 ml-auto" />}
          </div>
        )}

        {/* Credibility badges */}
        {(fake_news_score != null || (political_bias && political_bias !== 'neutral' && BIAS_CONFIG[political_bias]) || is_militant || has_incoherence) && (
          <div className="flex flex-wrap gap-1 mb-3">
            {fake_news_score != null && (
              <span className={`px-1.5 py-0.5 rounded-md text-xs font-semibold border ${fakeNewsColor(fake_news_score)}`}
            ...
            {political_bias && political_bias !== 'neutral' && BIAS_CONFIG[political_bias] && (
              <span className={`px-1.5 py-0.5 rounded-md text-xs font-semibold border ${BIAS_CONFIG[political_bias].color}`}>
            ...
            {has_incoherence && (
              <span className="px-1.5 py-0.5 rounded-md text-xs font-semibold border bg-yellow-500/15 text-yellow-400 border-yellow-500/40"
          ...
          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150 flex-shrink-0">
            ...
            <button
              ...
              className={`p-1.5 rounded-lg hover:bg-accent-primary/15 transition-colors ${
                bookmarked ? 'text-accent-primary' : 'text-text-secondary hover:text-accent-primary'
              }`}
