      ...
      `}
			>
				<div className={`h-full flex flex-col ${isCollapsed ? "p-3" : "p-6"}`}>
					<div
						className={`flex items-center ${isCollapsed ? "justify-center" : "justify-between"} mb-8 relative`}
					>
						{!isCollapsed ? (
						...
						{categories.map((category) => {
							...
							return (
								<button
									...
									className={`
                    w-full flex items-center ${isCollapsed ? "justify-center px-0" : "gap-3.5 px-3.5"} py-2.5 rounded-[14px] text-sm font-semibold transition-all duration-200 group relative
                    ${
											isSelected
												? "bg-blue-600 text-white shadow-md shadow-blue-600/20 dark:shadow-none"
                  ...
                  `}
								>
									<Icon
										size={20}
										className={`transition-colors shrink-0 ${isSelected ? "text-white" : "text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300"}`}
									/>
