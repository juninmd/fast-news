...
const Settings = ({
	...
	const addFeed = () => {
		...
		setCustomFeeds([
			...customFeeds,
			{ url: newFeedUrl.trim(), category: newFeedCategory },
		]);
		setNewFeedUrl("");
			...
			<div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-800">
				...
				<div className="flex-1 space-y-6 overflow-y-auto p-6">
					{activeTab === "geral" && (
						<>
							<Field
								label="RSS2JSON API Key (opcional)"
								value={rss2jsonApiKey}
								onChange={setRss2jsonApiKey}
								placeholder="Melhora limites de requisicao RSS"
							/>
							<label className="flex items-center justify-between rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-700/30">
								<span>
									...
									<span className="text-xs text-gray-500 dark:text-gray-400">
										Usa a API backend de IA configurada no servidor.
									</span>
								</span>
								<input
									type="checkbox"
						...
						<div className="space-y-4">
							<div className="grid gap-3 sm:grid-cols-[1fr_140px_auto]">
								<input
									...
									className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-600 dark:bg-gray-700/50"
								/>
								<input
							...
							{customFeeds.map((feed, index) => (
								<div
									key={`${feed.url}-${index}`}
									className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-700/30"
								>
				...
				<div className="flex justify-between border-t border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
					<button
						type="button"
						onClick={() => setCustomFeeds([])}
						className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700"
					>
	...
	return (
		<label className="block">
			...
			<input
				...
				className="w-full rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm dark:border-gray-600 dark:bg-gray-700/50"
			/>
		</label>
	);
}
