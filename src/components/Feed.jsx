...
import { FEED_SOURCES, fetchNews } from "../services/newsService";
...
const Feed = ({
	...
	const loadMoreNews = useCallback(
		async (isRetry = false) => {
			...
			try {
				const nextSources = currentSources.slice(
					nextBatchIndex,
				...
				const newNews = await withRetry(
					() => fetchNews(nextSources, rss2jsonApiKey),
					{
						...
						onRetry: (attempt, err) => {
							console.warn(
								`[Feed] Retry attempt ${attempt} after error:`,
								err.message,
							);
				...
				{loading && !isLoadingInitial ? (
					...
					filteredNews.length > 0 && (
						<p className="text-gray-400 dark:text-gray-500 text-sm">
							Todas as fontes disponíveis foram carregadas.
						</p>
					)
				)}
			</div>
