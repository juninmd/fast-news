...
describe('NewsCard', () => {
    ...
    it('handles summarize action', async () => {
        ...
        render(<NewsCard item={mockItem} ollamaUrl="http://test-url" ollamaModel="llama3" />);
    ...
    it('shows error if ollama url is missing', () => {
         render(<NewsCard item={mockItem} ollamaUrl="" />);

         const summarizeButton = screen.getByTitle('Resumir com Ollama');
         fireEvent.click(summarizeButton);

         expect(screen.getByText('Configure o Ollama nas configurações.')).toBeInTheDocument();
    ...
    it('triggers auto summarization if enabled', async () => {
        ...
        render(<NewsCard item={mockItem} ollamaUrl="http://test-url" autoSummarize={true} />);
