					...
					{bookmarks.length === 0 ? (
						<div className="text-center py-16">
							...
							<p className="text-gray-400 dark:text-gray-500 text-sm mt-2">
								Clique no ícone de bookmark para salvar artigos
							</p>
						</div>
					) : (
						<div className="space-y-4">
							{bookmarks.map((item) => (
								<div
									key={item.id}
									className="flex gap-4 p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl border border-gray-100 dark:border-gray-700 group hover:border-blue-200 dark:hover:border-blue-800 transition-colors"
								>
									<div className="flex-1 min-w-0">
										...
										<div className="flex items-center gap-4 text-xs text-gray-400 dark:text-gray-500">
											<span className="flex items-center gap-1">
												<Clock size={12} />
												{estimateReadTime(item.title)} min de leitura
											</span>
											<span className="flex items-center gap-1">
												<Calendar size={12} />
