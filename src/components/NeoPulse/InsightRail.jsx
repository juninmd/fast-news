...
export function InsightRail({ articles, analyses, onFilter }) {
	...
	const topSources = [
		...new Map(articles.map((item) => [item.source, item])).keys(),
	].slice(0, 5);
	const best = articles.filter((item) => item.usefulScore >= 70).slice(0, 3);
	const insights = Object.values(analyses)
		...
		<aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
				...
				<div className="flex flex-wrap gap-2">
					{terms.map(([term, count]) => (
						<button
							...
							className="rounded-md bg-cyan-300/10 px-2.5 py-1.5 text-xs font-bold text-cyan-700 hover:bg-cyan-300/20 dark:text-cyan-100"
						>
			...
			<section className="rounded-lg border border-ink bg-panel p-4">
				...
				<div className="space-y-3">
					{best.map((item) => (
						...
						>
							<span className="text-sm font-black leading-tight text-strong">
								{item.title}
							</span>
							<span className="mt-1 block text-xs text-faint">
								{item.source} - score {item.usefulScore}
							</span>
						</button>
					))}
				</div>
			...
			<section className="rounded-lg border border-ink bg-panel p-4">
				...
				<div className="space-y-3 text-sm leading-6 text-muted">
					{insights.length ? (
						insights.map((item) => <p key={item.why}>{item.why}</p>)
					) : (
						<p>Aguardando a API backend analisar os primeiros sinais uteis.</p>
					)}
				</div>
			</section>
			<section className="rounded-lg border border-ink bg-panel p-4">
				...
				<div className="space-y-2">
					{topSources.map((source) => (
						<p key={source} className="truncate text-sm text-muted">
							{source}
						</p>
					))}
				</div>
