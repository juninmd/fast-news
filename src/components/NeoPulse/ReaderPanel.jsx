			...
			<div className="p-6">
				...
				{fullLoading && (
					<p className="mt-4 flex items-center gap-2 text-sm text-cyan-200">
						<Loader className="h-4 w-4 animate-spin" /> Buscando post
						completo...
					</p>
				)}
				{analysis && (
					<div className="my-5 rounded-lg border border-cyan-400/20 bg-cyan-400/10 p-4">
						<p className="text-sm font-bold text-cyan-100">
							Utilidade: {analysis.useful ? "alta" : "baixa"} · {analysis.score}
							/100
						</p>
				...
				{analysis?.actions?.length > 0 && (
					<div className="mt-8 grid gap-3 sm:grid-cols-2">
						{analysis.actions.map((item) => (
							<div
								key={item}
								className="rounded-lg bg-white/5 p-3 text-sm text-zinc-200"
							>
								{item}
							</div>
						))}
						{analysis.risks.map((item) => (
							<div
								key={item}
								className="rounded-lg bg-amber-300/10 p-3 text-sm text-amber-100"
							>
								{item}
							</div>
						))}
