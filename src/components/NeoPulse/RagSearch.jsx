...
function ScoreBadge({ score }) {
  ...
  const color = pct >= 80 ? 'text-emerald-300' : pct >= 60 ? 'text-amber-300' : 'text-zinc-400';
...
export function RagSearch({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-start bg-black/80 pt-16 backdrop-blur-sm" onClick={onClose}>
      <div
      ...
      >
        <div className="flex items-center gap-3 border-b border-white/10 p-4">
          ...
          <button onClick={onClose} className="ml-auto neo-icon"><X className="h-4 w-4" /></button>
        </div>
        ...
        {results && (
          <div className="max-h-[60vh] overflow-y-auto px-4 pb-4">
            <p className="mb-3 text-xs text-zinc-500">{results.total} resultados para "{results.query}"</p>
              ...
              {results.results.map((item) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold leading-tight text-white line-clamp-2">{item.title}</p>
                    <p className="mt-1 line-clamp-2 text-xs text-zinc-400">{item.content?.slice(0, 140)}</p>
        ...
        {results?.results?.length === 0 && (
          <p className="px-4 pb-4 text-sm text-zinc-500">Nenhum resultado. Aguarde a ingestão popular o banco.</p>
        )}
      </div>
    </div>
  );
