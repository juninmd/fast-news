...
const stat = (label, value, Icon, tone) => (
	<div className="rounded-lg border border-ink bg-panel p-3">
		<div
			className={`mb-3 flex h-8 w-8 items-center justify-center rounded-md ${tone}`}
		>
	...
	return (
		<section className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
				...
				<div className="relative h-full p-6">
					...
					<div className="relative grid h-full content-between gap-8">
						<div className="max-w-3xl">
							<p className="mb-3 text-xs uppercase tracking-[0.32em] text-cyan-200">
								Sinal principal
							</p>
							<h2 className="text-3xl font-black leading-tight text-white sm:text-5xl">
								{lead?.title || "Carregando fontes"}
							</h2>
							<p className="mt-4 max-w-xl text-sm leading-6 text-zinc-300">
								{lead?.excerpt ||
									"Lendo RSS, cruzando sinais e preparando triagem."}
							</p>
						</div>
						<div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
							<div>
								<p className="text-xs uppercase tracking-[0.22em] text-zinc-500">
									{lead?.source || "Fast News"}
								</p>
			...
			<div className="grid grid-cols-2 gap-3">
				...
				{indexed > 0 &&
					stat("indexadas", indexed, Brain, "bg-violet-400 text-zinc-950")}
				<div className="col-span-2 rounded-lg border border-ink bg-panel p-4">
					...
					<div className="flex flex-wrap gap-2">
						{[...new Set(hot)].map((item) => (
							<span
								key={item}
								className="rounded-md bg-cyan-300/10 px-2 py-1 text-xs text-cyan-700 dark:text-cyan-100"
							>
								{item}
							</span>
						))}
