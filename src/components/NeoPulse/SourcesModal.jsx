...
export function SourcesModal({ sources, onClose }) {
	...
	return (
		<div className="fixed inset-0 z-50 bg-black/75 p-4" onClick={onClose}>
			<section
				className="mx-auto flex h-full max-w-5xl flex-col rounded-lg border border-white/10 bg-zinc-950"
				...
				<header className="flex items-center gap-3 border-b border-white/10 p-4">
					<div>
						...
						<p className="text-sm text-zinc-400">
							{filtered.length} de {sources.length} feeds RSS
						</p>
					</div>
				...
				<div className="overflow-y-auto p-4">
					<div className="grid gap-4 md:grid-cols-2">
						{Object.entries(grouped).map(([category, items]) => (
							...
							>
								<h3 className="mb-3 font-black text-cyan-100">
									{category}{" "}
								...
								<div className="space-y-2">
									{items.map((item) => (
										<a
											...
											target="_blank"
											rel="noreferrer"
											className="flex items-center gap-2 rounded-md bg-white/5 px-3 py-2 text-sm text-zinc-300 hover:text-cyan-200"
										>
