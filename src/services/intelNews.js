...
const scoreArticle = (item) => {
	const text =
		`${item.title} ${cleanText(item.description || item.content)}`.toLowerCase();
	const useful = [
		"lança",
		"anuncia",
		"alerta",
	...
	const fresh =
		Date.now() - new Date(item.pubDate).getTime() < 36 * 60 * 60 * 1000;
	return (
		useful.filter((w) => text.includes(w)).length * 12 +
		strategic.filter((w) => text.includes(w)).length * 10 +
		(fresh ? 24 : 0) +
		Math.min(cleanText(item.description).length / 18, 24)
	);
};
...
export async function fetchIntelBatch({
	...
	const filtered =
		category === "Todas" ? pool : pool.filter((s) => s.category === category);
	const sources = shuffle(filtered).slice(offset, offset + size);
	const raw = await fetchNews(sources, rssKey);
	const unique = new Map(
		raw.map((item) => [item.guid || item.link, normalizeArticle(item)]),
	);
	return {
